...
def set_page_info():
    st.set_page_config(layout='wide', page_title='Product Analysis', page_icon='./logo/UTY_logo_ORI.png',)
    new_title = '<p style="font-family:sans-serif; color:green; font-size: 42px;">PHÂN TÍCH SẢN PHẨM THEO LOẠI SẢN PHẨM</p>'
    st.markdown(new_title, unsafe_allow_html=True)
    st.text("")

def show_data(data):
    column_configuration = {
        "image_link": st.column_config.ImageColumn("image_link", help="image_link From Shopee"),
    }
    st.data_editor(
            data,
...
def plot_category(selected_df):
    ...
    groups_count = selected_df.groupby("sub_category")["product_name"].agg("count").reset_index()[["sub_category", "product_name"]]
    groups_count = groups_count.sort_values(by="product_name", ascending=False)
    ...
    selected_columns = ["brand", "shop_name", "product_name", "price_vnd", "category", "sub_category", "sold_per_day","revenue_per_day_vnd", "image_link"]
...
def plot_pie_brand(data, raw_data):
    ...
    #     fig = px.pie(data, values='revenue_per_day_vnd', names='brand', title='Phân bổ brand dựa vào doanh thu')
...
def plot_brand_count(data, raw_data):
    ...
    brand_count_total = get_raw_data_total.groupby("brand")["Sold/M"].agg("sum").reset_index().sort_values(by="Sold/M", ascending=False)    
    brand_count_cat = get_raw_data_category.groupby("brand")["Sold/M"].agg("sum").reset_index().sort_values(by="Sold/M", ascending=False)    
    
    

    fig1 = brand_count_total.plot(kind="barh", x="Sold/M", y="brand", 
...
def page():
    ...
    if filtered_data is not None:
        ...
        st.markdown("""<hr style="height:10px;border:none;color:#34256;background-color:#333;" /> """, unsafe_allow_html=True)
        plot_category(selected_df)
